...
$category_id = isset($queried_object->term_id) ? $queried_object->term_id : 0;

// 3. 用 WP_Query 查該分類總筆數（只抓 ID，減輕效能負擔）
$wp_query = new WP_Query([
    'post_type'      => 'new',
    'posts_per_page' => 1,  // 為了能正確取得 found_posts
    'fields'         => 'ids',
    'post_status'    => 'publish',
    'tax_query'      => [
        [
...
$total = $wp_query->found_posts; // 總筆數
$total_pages = ceil($total / $per_page); // 總頁數

// 4. 用 Pods 查實際資料（搭配 offset + limit）
$works = pods('new', [
    'limit'  => $per_page,
    'offset' => $offset,
    'where'  => "new_category.term_id = $category_id",
]);
	...
	<div class="bg-[#efefef] xl:zoom-90">
		<div class="max-w-[1505px] mx-auto pt-[270px] pb-[252px] xl:max-w-[1100px] lg:px-7 lg:pt-[72px] lg:pb-[162px]">
			<div class="flex justify-between mb-[152px] lg:block lg:mb-32">
				...
				<div class="lg:mt-5">
					<ul class="space-y-4 lg:flex lg:items-center lg:space-y-0 lg:space-x-5">
						<li class="relative pr-[50px] group lg:shrink-0 lg:pr-5"><a href="<?= home_url() ?>/new">
						...
						<?php $active_class = ($category_id == $c->term_id) ? 'current' : ''; ?>
							<li class="relative pr-[50px] group lg:shrink-0 lg:pr-5 <?= esc_attr($active_class) ?>"><a href="<?= esc_url(get_term_link($c->slug, 'new_category')) ?>"> 
			...
			<div class="">
				...
				<!-- <div class="news-paper flex justify-center mt-[220px] lg:mt-[110px]">
					<div class="flex items-center">
						<a href="" class="mr-[112px] lg:mr-10">
						...
						<a href="" class="ml-[112px] lg:ml-10">
							<svg width="137.15" height="3" viewBox="0 0 137.15 3" class="lg:hidden">
								<line x1="137.15" y1="1.63" y2="1.63" style="fill: none; stroke: #595757; stroke-miterlimit: 10; stroke-width: .75px;"/>
								<line x1="82.3" y1="1.5" x2="137.15" y2="1.5" style="fill: none; stroke: #005188; stroke-miterlimit: 10; stroke-width: 3px;"/>
							</svg>
