...
$works = pods('gallery', [
    'limit' => -1,
    'where' => "gallery_category.term_id = $category_id" // 只篩選屬於當前分類的內容
]);
	...
	<template id="refract-template">
		<div class="refract-area lg:pt-[104px]">
			...
			<div class="mt-[175px] mb-[145px] lg:mt-0 lg:mb-32"><a href="<?= esc_url(get_permalink($first->ID)) ?>" class="flex mouse-arrow lg:block">
				...
				<div class="flex-auto"><img src="<?= esc_url(get_image_url($first->ID, 'gallery_cover')); ?>"></div>
			</a></div>
			<div class="">
				<div class="flex justify-between mb-[180px] lg:block lg:mb-20">
					...
					<div class="flex-auto h-max">
						<ul class="cats font-en text-[#595757] text-[21px] flex justify-end space-x-4 mt-10 mb-6 lg:justify-start lg:space-x-4 lg:text-[14px] lg:mb-[82px]">
							<li class="pl-7 hover:bg-[#115992] hover:text-white [&.current]:bg-[#115992] [&.current]:text-white lg:pl-0"><a href="<?= home_url() ?>/gallery">ALL</a></li>
							<?php foreach($cats->rows as $c) : ?>
							<?php $active_class = ($category_id == $c->term_id) ? 'current' : ''; ?>
								<li class="pl-7 hover:bg-[#115992] hover:text-white [&.current]:bg-[#115992] [&.current]:text-white lg:pl-0 <?= esc_attr($active_class) ?>">
						...
						<li v-for="p in posts" class="mouse-arrow"><a :href="p.link">
							...
							<div class="flex">
								...
								<div class="shrink-0 text-right">
									...
									<div class="font-en text-[#898989] text-[13px]">{{p.mon}}.<span class="font-num">{{p.date}}</span></div>
								</div>
							</div>
						</a></li>
					</ul>
